...
function cariModus(arr) {
    ...
    for (let num of arr) { //[10, 4, 5, 2, 4]
        freq[num] = (freq[num] || 0) + 1; //1, 1,
        if (freq[num] > maxCount) maxCount = freq[num];
    }

    // Jika semua angka sama atau tidak ada modus
    if (maxCount === arr.length || maxCount === 1) return -1;

    // Cari modus pertama yang muncul dengan frekuensi maxCount
    ...
    // Jika hanya ada satu nilai unik dan muncul lebih dari sekali, return -1
