...
function shoppingTime(memberId, money) {
  // you can only write your code here!
    ...
    let ListPurchased = [];
    if (memberId === '' || !memberId) return 'Mohon maaf, toko X hanya berlaku untuk member saja';
    if (money < 50000 || !money) return 'Mohon maaf, uang tidak cukup';
    let ListBarang = {
    ...
    let SortedListBarang = Object.entries(ListBarang).sort((a, b) => b[1] - a[1]);
    for (let key in ListBarang){
        if (ListBarang[key] <= changeMoney){
            ListPurchased.push(key);
            changeMoney -= ListBarang[key];
        }
    }
